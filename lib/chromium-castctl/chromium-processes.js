...
async function persistLaunchedBrowserIdentity(paths, child, configuredExecutable, launchToken, deadline) {
  ...
  while (Date.now() < deadline) {
    ...
    const args = cmdlineArgs(processIdentity);
    ...
    if (processIdentity?.startTime
      && args.includes(`--chromium-castctl-launch-token=${launchToken}`)) {
...
function processIsProfileBrowser(identity, paths, env = process.env) {
  ...
  if (!args.includes(`--user-data-dir=${paths.profileDir}`) || args.some((arg) => arg.startsWith('--type='))) {
    return false;
  }
  if (actualExecutable === configuredExecutable) return true;

  return matchesLaunch && args.includes(`--chromium-castctl-launch-token=${launched.launchToken}`);
}
